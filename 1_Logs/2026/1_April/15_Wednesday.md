# 📅 Wednesday, 15 April 2026

## 🚀 Engineering & Skills Log

### 🔵I. Human OS : (Non-Negotiable: Maintenance)
- **German 🇩🇪**
    - **Focus:** updated the entire german system. moved the old v1 to archive. it can be viewed, but not relied upon anymore.
    - **Progress:** created v2 system,. find the notes below.
    - **Detailed Notes:** [daily capture note](../../../2_Knowledge_Vault/3_Learning_Deutsch_v2/0_daily_capture_note.md), [vocab master fiile](../../../2_Knowledge_Vault/3_Learning_Deutsch_v2/1_German_Vocab_master.md), [grammar codex](../../../2_Knowledge_Vault/3_Learning_Deutsch_v2/2_German_grammar_codex.md), [immersion tracker](../../../2_Knowledge_Vault/3_Learning_Deutsch_v2/3_German_immersion_tracker.md), [output and practice](../../../2_Knowledge_Vault/3_Learning_Deutsch_v2/4_Output_&_practice.md) files were created and some are still under experiment.
ㅤㅤ
- **Reading & Research** (*Review session* : target 50 pages/day)  
    - **Psychology of money** *REVIEW*  : Not much due to redesigning a messy german learning system.
	    - progress :
		    - 111-120 done
    - **Article/Paper:** NA  
ㅤㅤ
 - **Action Items zetteling**
	 - **Deep work** *progress* : Not necessary today.
ㅤㅤ
- **Private university prep**
	- **a's book completion progress** :Done till page 202. Tomorrow, I will have to catch up with my quote.

### 🟢 II. THE ENGINE : (Non-Negotiable: Daily Foundation)

- **CS50x  & C programming💻**
    - **Topic:** Solved grayscale and reflect functions alongside the environment issues(windows stupidity).
    - **Notes:** [source code](../../../3_Projects_and_Labs/1_CS50x/02-Problem-sets/Pset_4.2/filter-more/helpers.c), [readme](../../../3_Projects_and_Labs/1_CS50x/02-Problem-sets/Pset_4.2/readme.md)
ㅤㅤ
- **Linear Algebra for ML & DS** 📐
	- **Topic:** Jupyter matplotlib made me frustrated. This is the worst obsidian plugin i have seen, i have to turn it on in vs code. Not much done today.
	- **Notes:** [the .md file](../../../2_Knowledge_Vault/2_Math/Notes/1_week-1/15%20Linear%20systems%20as%20matrices.md) and [the jupyter file](../../../2_Knowledge_Vault/2_Math/Notes/1_week-1/15%20Linear%20systems%20as%20matrices.ipynb)
ㅤㅤ
- **NumPy:** still stalled. not much progress except for basic addition and prob solving.
ㅤㅤ
### 🟡 III. THE TOOLBELT (Applied: Project-Based Only)

- **Microsoft Excel** (currently learning)
	- **Topic:** Done with excel for today, the 6th video is the trickiest and hardest. I need to see and use excel more, in order to understand toobar custm. better.
	- **Notes:** [click here for the detailed note.](../../../2_Knowledge_Vault/1_MS_Excel/ExcelIsFun_YT_course/06%20Excel%20Basics%206%20Customize%20Quick%20Access%20Toolbar%20(QAT)%20&%20Show%20New%20Ribbon%20Tabs.md)
ㅤㅤ
### 🟣 IV. THE PROFESSIONAL LAYER (Milestone-based)

- 

### 🟠 V. THE PLAYGROUND (Controlled Chaos)

- 